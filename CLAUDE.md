...
Expo SDK 57 app (own package root, NOT an npm workspace member — avoids
React 18/19 hoisting conflicts with the desktop app). Consumes `theory-core`
via a `file:../packages/theory-core` link; Metro watches it (`metro.config.js`).
Managed workflow: `ios/`/`android/` are generated by `npx expo prebuild`, never
committed, never hand-edited — all native config is in `app.json` plus the
local Expo module `mobile/modules/midi-ble-pairing/` (Bluetooth MIDI pairing
sheet). MIDI comes from `@motiz88/react-native-midi` (Web MIDI API shape, so
MIDI-consuming code matches the desktop renderer's). iOS builds require macOS:
CI (`.github/workflows/ios-ci.yml`) compiles unsigned on every mobile/packages
change; signed device builds are done by a collaborator per `mobile/BUILD.md`.
Port plan: `../the-derple-dex/plans/the-tightening-ios-port.md`.
